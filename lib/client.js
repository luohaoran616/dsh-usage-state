window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*
		* Fork note (dsh 0.1.7): the dock lays entries out in a flex ROW, so upstream's
		* `width: '100%'` demanded the whole row width and folded the native stats pills.
		* Shrink to content, cap the share, and push the reading to the row's right end.
		*/
		const DOCK_STYLE = {
			display: "flex",
			alignItems: "center",
			...
			minWidth: 0,
			marginLeft: "auto",
			padding: "0 calc(var(--dsh-composer-side-clearance) + 16px) 0"
		};
		...
		*/
		function StatusLine(props) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				style: DOCK_STYLE,
				children: fullParts.map((part, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react.Fragment, { children: [index > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
		...
		*/
		...
		function apply(ctx) {
			...
			const httpGet = async (path) => {
				try {
					return await (await fetch(`${API_PREFIX}${path}`, { headers: { accept: "application/json" } })).json();
				} catch (error) {
					return {
						ok: false,
			...
			const store = new UsageStateClientStore({
				getState: (force) => httpGet(`/state?force=${force ? 1 : 0}`),
				describeCredentials: () => httpGet("/credentials"),
				modelCatalog: async () => {

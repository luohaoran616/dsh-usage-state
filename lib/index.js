...
function registerHttpRoutes(ctx, service) {
			...
			handler: async (req, res) => {
				...
				try {
					if (url.pathname === `${API_PREFIX}/state`) {
						...
						return send(200, {
							ok: true,
							value: await service.getState(force)
						});
					}
					if (url.pathname === `${API_PREFIX}/credentials`) return send(200, {
						ok: true,
						value: await service.describeCredentials()
					});
					return send(404, {
						ok: false,
...
function resolveTargets(config, options = {}) {
	...
	*/
	const live = options.providers !== void 0 && options.providers.length > 0 ? new Set(options.providers) : void 0;
	/**
	...
	*/
	...
	const store = new UsageStateStore({
		...
		credentials: { resolve: async (target) => {
			...
			if (baseUrl === void 0) return resolved;
			return {
				...resolved,
				baseUrl,
				...pinnedBaseUrl === void 0 ? {} : { baseUrlPinned: true }
			};
		} },
